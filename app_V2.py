# app.py —— 昆岛（Côn Đảo）气象系统 （最新版）
# 包含：天气预报 / METAR解析 / 降水过程记录 / 历史分析
...
def page_rain():
    ...
    time_str = st.text_input("时间（YYYY-MM-DD HH:MM）例如：2025-01-12 12:05")

    rain_level = st.selectbox(
        "雨强",
        ["小雨", "中雨", "大雨", "暴雨", "雷阵雨", "毛毛雨", "雨停"],
    )

    rain_code = st.text_input("对应报文代码（可选）")
    ...
    if st.button("查询降水历史"):
        ...
        st.dataframe(df, use_container_width=True)
        st.caption("📌 上表为完整降水过程记录（开始→增强→减弱→雨停）")
